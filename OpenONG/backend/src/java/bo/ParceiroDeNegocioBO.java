...
public class ParceiroDeNegocioBO {
    ...
    private List<Erro> validacoes(ParceiroDeNegocio pn, Session session) {
        ...
        if (pn.getNome() == "" || pn.getNome() == null || pn.getNome().isEmpty()) {
            ...
            boolean exists = new ParceiroDeNegocioDAO().parceiroExists(pn.getNome(), session);
        ...
        if (pn.getCnpj() != null) {
            if (pn.getCnpj().length() == 14 || pn.getCnpj().length() == 18) {
                boolean exists = new ParceiroDeNegocioDAO().cnpjExists(pn.getCnpj(), session);
                if (exists) {
        ...
        if (pn.getCpf() != null) {
            if (pn.getCpf().length() == 11 || pn.getCpf().length() == 14) {
                boolean exists = new ParceiroDeNegocioDAO().cpfExists(pn.getCpf(), session);
                if (exists) {
